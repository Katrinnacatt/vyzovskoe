...
int main() {
    ...
    vector<int> totalEmployees(3, 0); // Вектор для хранения общего количества сотрудников
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        while (getline(namesFiles[i], name)) {
            ...
            totalEmployees[i]++; // Увеличение количества сотрудников
        }
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        // Вычисление среднего значения уплаченных взносов
        double averagePayment = static_cast<double>(totalPayments[i]) / totalEmployees[i];
        result << "----------------------------------------------------" << endl;
        result << "Среднее значение уплаченных взносов: " << averagePayment << endl;
        result << endl;
    }
