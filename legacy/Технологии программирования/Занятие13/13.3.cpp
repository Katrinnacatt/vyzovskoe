...
int main() {
    ...
    vector<int> maxPayments(3, INT_MIN); // Вектор для хранения максимального значения уплаченных взносов
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        while (paymentsFiles[i] >> payment) {
            ...
            maxPayments[i] = max(maxPayments[i], payment); // Находим максимальный взнос для текущего подразделения
        }
    }
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        // Вывод максимального значения уплаченных взносов для текущего подразделения
        result << "----------------------------------------------------" << endl;
        result << "Максимальное значение уплаченных взносов: " << maxPayments[i] << endl;
        result << endl;
    }
